...
def _():
    ...
    return BACKEND, Path, cp, h5py, ipv, mrinufft, np, plots_dir, plt, pp
...
def _(output_dir, pp):
    ...
    sequence.read(output_dir / 'sequences' / '....seq')
    print(sequence)
    return (sequence,)
...
def _(
    ...
    _axs[0].imshow(abs(adjoint_manual)[adjoint_manual.shape[0] // 2 + 1, :, ::-1][24:-24, 20:-20], cmap='viridis')
    _axs[1].imshow(abs(adjoint_manual)[::-1, adjoint_manual.shape[1] // 2+1, :][18:-18, 20:-20], cmap='viridis')
    _axs[2].imshow(abs(adjoint_manual)[::-1, :, adjoint_manual.shape[2] // 2+1].T[24:-24, 18:-18], cmap='viridis')

    for _ax in _axs:
        _ax.axis('off')

    _fig.savefig(plots_dir / 'koma_static.svg', transparent=True)
...
def _(
    ...
    _fig, _axs = plt.subplots(1, 3, figsize=(15, 5))
    _axs[0].imshow(abs(adjoint_manual_subs)[adjoint_manual_subs.shape[0] // 2 + 1, :, ::-1][24:-24, 20:-20], cmap='viridis')
    _axs[1].imshow(abs(adjoint_manual_subs)[::-1, adjoint_manual_subs.shape[1] // 2+1, :][18:-18, 20:-20], cmap='viridis')
    _axs[2].imshow(abs(adjoint_manual_subs)[::-1, :, adjoint_manual_subs.shape[2] // 2+1].T[24:-24, 18:-18], cmap='viridis')

...
def _(Path, h5py, np):
    ...
    simulation_file = h5py.File(output_dir / 'simulation_outputs' / '....mat')
    coil_signals = np.array(simulation_file.get('raw'))
    coil_signals = np.squeeze(coil_signals)
    
    coil_signals = coil_signals['real'][:] + 1j * coil_signals['imag'][:]
...
def _(BACKEND, coil_signals, dcf, mrinufft, normed_ktraj, output_shape, plt):
    ...
    _axs[0].imshow(abs(adjoint_manual)[adjoint_manual.shape[0] // 2 + 1, :, ::-1][20:-20, 20:-20], cmap='viridis')
    _axs[1].imshow(abs(adjoint_manual)[::-1, adjoint_manual.shape[1] // 2+1, :][18:-18, 20:-20], cmap='viridis')
    _axs[2].imshow(abs(adjoint_manual)[::-1, :, adjoint_manual.shape[2] // 2+1].T[20:-20, 18:-18], cmap='viridis')
...
def _(
    ...
    _fig, _axs = plt.subplots(1, 3, figsize=(15, 5))
    _axs[0].imshow(abs(adjoint_manual_subs)[adjoint_manual_subs.shape[0] // 2 + 1, :, ::-1][20:-20, 20:-20], cmap='viridis')
    _axs[1].imshow(abs(adjoint_manual_subs)[::-1, adjoint_manual_subs.shape[1] // 2+1, :][18:-18, 20:-20], cmap='viridis')
    _axs[2].imshow(abs(adjoint_manual_subs)[::-1, :, adjoint_manual_subs.shape[2] // 2+1].T[20:-20, 18:-18], cmap='viridis')
...
def _(breath_phases, breath_time, np, phase_resolution, rendered_frames):
    ...
    imwrite(
        outfile_anim,
        255 * np.abs(rendered_frames)[:, rendered_frames.shape[0] // 2 + 1, :, ::-1][20:-20, 20:-20] / np.max(np.abs(rendered_frames)),
...
def _(
    ...
    for _phase, time_sample in enumerate(255 * (np.abs(rendered_frames) / np.max(np.abs(rendered_frames)))):
        ...
        time_axs[0].imshow(np.abs(time_sample)[time_sample.shape[0] // 2 + 1, :, ::-1][20:-20, 20:-20], cmap='gray', vmax=clip_value, vmin=0)
        time_axs[1].imshow(np.abs(time_sample)[::-1, time_sample.shape[1] // 2+1, :][18:-18, 20:-20], cmap='gray', vmax=clip_value, vmin=0)
        time_axs[2].imshow(np.abs(time_sample)[::-1, :, time_sample.shape[2] // 2+1].T[20:-20, 18:-18], cmap='gray', vmax=clip_value, vmin=0)
